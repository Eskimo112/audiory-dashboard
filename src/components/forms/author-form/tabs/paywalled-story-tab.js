...
import { Box, Button, Container, Grid, MenuItem, Skeleton, Stack, Switch, TextField, Typography } from "@mui/material";
...
const PaywalledStoryTab = ({ story, handleRefetch }) => {
    ...
    const handleDialogPriceModalClose = async (isConfirm) => {
        ...
        if (isConfirm) {
            try {
                await new StoryService(requestHeader).paywall({ storyId: story?.id, price: formik.values.chapter_price }).then(res => {
                    if (res.code === 200) {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            chapter_price: Yup.number().min(1, 'Ít nhất 1 xu').max(10, 'Nhiều nhất 10 xu').required(),
        }),
                        ...
                        <ConfirmDialog
                            ...
                            content={isLoadingCriteria ? <Skeleton /> : <Grid container spacing={0}>
                                <Grid container spacing={0}>
                                    <Typography variant="subtitle1" color={allCheckedCriteria ? 'primary' : "secondary"}>
                                        {allCheckedCriteria ? `Xin chúc mừng, tác phẩm ${story?.title} đạt đủ điền kiện thương mại hóa` : `Xin lỗi, tác phẩm ${story?.title} hiện chưa đáp ứng đủ điều kiện thương mại hóa`}
                                    </Typography>
                                </Grid>
                                {
                                    criteriaData.message?.map((criteria, index) => (
                                        <Grid key={index} container direction="column" sx={{ padding: "0.5em 2em" }}>
                                            <Grid container spacing={0} >
                                                <Grid xs={1}>
